      ...
      <div class="student-dashboard-modern" style="width:100%; max-width:1440px; margin:0 auto; padding:24px 20px 80px; box-sizing:border-box;">
        ...
        <div style="display:grid; grid-template-columns: 1fr 360px; gap:28px; align-items:start;" class="dashboard-main-grid-layout">
          ...
          <div style="display:flex; flex-direction:column; gap:32px;">
            ...
            <!-- Section: Graded Assignments & Teacher Feedback (المصححة والملاحظات) -->
            ${gradedAssignments.length > 0 ? `
              <div class="glass-card" style="padding:24px; border-radius:22px; border:1px solid rgba(16,185,129,0.25); background:var(--bg-card); box-shadow:0 8px 24px rgba(16,185,129,0.06);">
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:16px; flex-wrap:wrap; gap:10px;">
                  <div>
                    <h3 style="font-size:1.15rem; font-weight:900; margin:0; color:var(--text-main); display:flex; align-items:center; gap:8px;">
                      <i data-lucide="award" style="width:22px; height:22px; color:#10b981;"></i>
                      الواجبات والمهام المصححة وملاحظات المعلم (${gradedAssignments.length}) 📝
                    </h3>
                    <p style="color:var(--text-muted); font-size:0.8rem; margin:3px 0 0 0;">نتائج الواجبات التي قام معلموك بتصحيحها ومشاركتها معك</p>
                  </div>
                  <a href="#student-groups" style="font-size:0.82rem; font-weight:700; color:var(--primary); text-decoration:none;">
                    مجموعاتي الدراسية ↗
                  </a>
                </div>

                <div style="display:flex; flex-direction:column; gap:12px;">
                  ${gradedAssignments.slice(0, 4).map(asg => {
                    const sub = asg.submission;
                    const grade = sub.grade !== null && sub.grade !== undefined ? sub.grade : 0;
                    const totalPts = asg.totalPoints || 100;
                    const pct = sub.percentage !== null && sub.percentage !== undefined ? sub.percentage : Math.round((grade / totalPts) * 100);
                    const isHigh = pct >= 80;
                    ...
                    const gradedDate = sub.gradedAt ? new Date(sub.gradedAt).toLocaleDateString('ar-EG', { month: 'short', day: 'numeric' }) : '';

                    return `
                      <div style="padding:16px 18px; border-radius:16px; background:var(--bg-app); border:1px solid var(--border-color); display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:14px;">
                        <div style="display:flex; align-items:center; gap:14px;">
                          <div style="width:52px; height:52px; border-radius:14px; background:${badgeBg}; color:${badgeColor}; display:flex; flex-direction:column; align-items:center; justify-content:center; font-weight:900; border:1.5px solid ${badgeColor}; flex-shrink:0;">
                            <span style="font-size:1.15rem; line-height:1;">${pct}%</span>
                            <span style="font-size:0.6rem; margin-top:2px;">النتيجة</span>
                          </div>
                          <div>
                            <div style="font-weight:900; font-size:0.95rem; color:var(--text-main);">${asg.title}</div>
                            <div style="font-size:0.78rem; color:var(--text-muted); margin-top:3px; display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
                              <span>📚 ${asg.course?.title || 'الدورة'}</span>
                              <span>•</span>
                              <span style="font-weight:800; color:var(--text-main);">الدرجة: <span style="color:${badgeColor}; font-weight:900;">${grade}</span> / ${totalPts}</span>
                              ${gradedDate ? `<span>• تم التصحيح: ${gradedDate}</span>` : ''}
                            </div>
                            ${sub.overallFeedback ? `
                              <div style="margin-top:6px; font-size:0.78rem; color:var(--text-main); background:var(--bg-card); padding:4px 10px; border-radius:8px; border:1px solid var(--border-color); display:inline-block; max-width:450px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">
                                💬 <strong>ملاحظة المعلم:</strong> ${sub.overallFeedback}
                              </div>
                            ` : ''}
                          </div>
                        </div>

                        <button class="btn-primary view-graded-assignment-report-btn" data-id="${asg.id}"
                          style="padding:8px 16px; font-size:0.82rem; font-weight:800; border-radius:12px; display:inline-flex; align-items:center; gap:6px; cursor:pointer;">
                          <i data-lucide="file-check-2" style="width:15px; height:15px;"></i>
                          <span>عرض تقرير وتصحيح الواجب 📋</span>
                        </button>
                      </div>
                    `;
                  }).join('')}
