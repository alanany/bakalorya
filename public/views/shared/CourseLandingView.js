...
import { openGroupPaymentModal } from "./GroupPaymentModal.js";

export default class CourseLandingView {
  constructor(container, routeParam) {
    this.container = container;
    // routeParam can be "courseId" or "courseId/groupId"
    const parts = (routeParam || '').split('/');
    this.courseId = parts[0];
    this.initialGroupId = parts[1] || null; // pre-selected group from URL
    this.course = null;
    this.groups = [];
  ...
  async render() {
    try {
      ...
      if (this.myEnrollments && this.myEnrollments.length > 0) {
        const myGroupEnrollment = this.myEnrollments.find(e => 
          e.course?.id === this.courseId && e.group?.id && String(e.group.id) === String(this.selectedGroupId)
        );
        const myCourseEnrollment = this.myEnrollments.find(e => e.course?.id === this.courseId && !e.group?.id);
        const myEnrollment = myGroupEnrollment || myCourseEnrollment;
        if (myEnrollment) {
          this.enrollmentStatus = myEnrollment.status || 'pending';
          this.enrolledGroupId = myEnrollment.group?.id;
        }
      }
      ...
      const isFull = selectedGroup && (selectedGroup.isFull || availableSeats <= 0);
      const mySelectedGroupEnrollment = (this.myEnrollments || []).find(e => 
        selectedGroup && e.group?.id && String(e.group.id) === String(selectedGroup.id)
      );
      const isPendingInSelected = mySelectedGroupEnrollment && (mySelectedGroupEnrollment.status === 'pending' || mySelectedGroupEnrollment.status === 'PENDING');
      const isEnrolledInSelected = mySelectedGroupEnrollment && (mySelectedGroupEnrollment.status === 'active' || mySelectedGroupEnrollment.status === 'ACTIVE');

      this.container.innerHTML = `
        <div style="background:var(--bg-color); min-height:100vh;">
          
          <!-- Top Cover (Slider / Hero) -->
          <div style="position: relative; width: 100%; min-height: 320px; background-image: url('${this.course.image || 'https://images.unsplash.com/photo-1516321318423-f06f85e504b3?w=1200'}'); background-size: cover; background-position: center; padding: 40px 16px;">
            <div style="position: absolute; inset: 0; background: linear-gradient(to top, rgba(0,0,0,0.92) 0%, rgba(0,0,0,0.4) 100%); display:flex; flex-direction:column; justify-content:center; align-items:center; text-align:center; padding: 24px 16px;">
              <div style="display:flex; gap:8px; align-items:center; margin-bottom:12px; flex-wrap:wrap; justify-content:center;">
                <span style="background: var(--primary); color: white; padding: 4px 14px; border-radius: 20px; font-size: 0.8rem; font-weight: 800; text-transform: uppercase;">
                  ${this.course.category || 'كورس تعليمي'}
                </span>
                ${this.course.grade ? `
                  <span style="background: rgba(16,185,129,0.2); color: #10b981; border: 1px solid #10b981; padding: 4px 12px; border-radius: 20px; font-size: 0.78rem; font-weight: 800;">
                    ${this.course.grade.name}
                  </span>
                ` : ''}
              </div>
          ...
          <!-- Main Content Section (2 columns: Left Content, Right Group Details & Description) -->
          <div style="max-width: 1200px; margin: 0 auto; padding: 50px 24px; display:flex; gap:36px; flex-wrap: wrap;">
            ...
            <!-- Right Column: Group Details Card (Matching Photo) & Teacher Info -->
            <div style="flex: 1; min-width: 320px;">
              
              <!-- Group Details Box (As in User Photo) -->
              <div class="glass-card" style="border-radius:22px; padding:24px 26px; border:1px solid var(--border-color); background:var(--bg-card); box-shadow:0 10px 30px rgba(0,0,0,0.06); margin-bottom:24px;">
                
                <!-- Group Switcher if course has multiple groups and NOT coming from a specific group link -->
                ${this.initialGroupId ? `
                  <!-- Single group view - came from specific group card -->
                  <div style="margin-bottom:16px; padding-bottom:14px; border-bottom:1px solid var(--border-color);">
                    <div style="display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:8px;">
                      <div style="font-weight:900; font-size:1rem; color:var(--text-main); display:flex; align-items:center; gap:8px;">
                        <i data-lucide="users" style="width:18px; height:18px; color:var(--primary);"></i>
                        <span>${selectedGroup?.name || `مجموعة ${selectedGroup?.scheduleDays || ''}`}</span>
                      </div>
                      ${mySelectedGroupEnrollment ? `
                        <div style="padding:5px 14px; border-radius:20px; font-weight:800; font-size:0.82rem;
                          ${isPendingInSelected 
                            ? 'background:rgba(245,158,11,0.12); border:1.5px solid #f59e0b; color:#d97706;' 
                            : 'background:rgba(16,185,129,0.12); border:1.5px solid #10b981; color:#059669;'}
                        ">
                          ${isPendingInSelected ? '⏳ قيد المراجعة' : '✓ مسجل'}
                        </div>
                      ` : ''}
                    </div>
                  </div>
                ` : this.groups.length > 1 ? `
                  <div style="margin-bottom:18px; padding-bottom:14px; border-bottom:1px solid var(--border-color);">
                    <label style="font-size:0.82rem; font-weight:800; color:var(--text-muted); display:block; margin-bottom:8px;">
                      اختر المجموعة الدراسية (${this.groups.length} مجموعات متاحة):
                    </label>
                    <div style="display:flex; flex-direction:column; gap:6px;">
                      ${this.groups.map(g => {
                        const isSelected = String(g.id) === String(selectedGroup?.id);
                        return `
                          <button type="button" class="course-group-picker-btn" data-group-id="${g.id}"
                            style="padding:10px 14px; border-radius:12px; text-align:start; cursor:pointer; font-family:'Cairo',sans-serif; transition:all 0.2s; border:1.5px solid ${isSelected ? 'var(--primary)' : 'var(--border-color)'}; background:${isSelected ? 'var(--primary-glow)' : 'var(--bg-app)'};">
                            <div style="font-weight:900; font-size:0.88rem; color:var(--text-main); display:flex; justify-content:space-between; align-items:center;">
                              <span>👥 ${g.name || `مجموعة ${g.scheduleDays || ''}`}</span>
                              <span style="font-size:0.75rem; font-weight:800; color:${g.availableSeats <= 3 ? 'var(--error)' : 'var(--primary)'};">
                                ${g.availableSeats !== undefined ? `(${g.availableSeats} مقعد متبقي)` : ''}
                              </span>
                            </div>
                            <div style="font-size:0.78rem; color:var(--text-muted); margin-top:2px;">
                              ${g.scheduleText || `${g.scheduleDays || ''} ${g.scheduleTime || ''}`}
                            </div>
                          </button>
                        `;
                      }).join('')}
                ...
                ` : selectedGroup ? `
                  <div style="margin-bottom:12px; font-weight:800; font-size:1rem; color:var(--text-main); display:flex; align-items:center; gap:8px;">
                    <i data-lucide="users" style="width:18px; height:18px; color:var(--primary);"></i>
                    <span>${selectedGroup.name || `مجموعة ${selectedGroup.scheduleDays || ''}`}</span>
                  </div>
                ` : ''}
                ...
                ${!selectedGroup ? `
                  <div style="text-align:center; padding:30px 10px; color:var(--text-muted);">
                    <i data-lucide="calendar" style="width:36px; height:36px; opacity:0.3; margin-bottom:8px;"></i>
                    <p style="margin:0; font-weight:700;">لا توجد مجموعات دراسية مفتوحة للتسجيل حالياً في هذا الكورس.</p>
                  </div>
                ` : `
                  <!-- Table Rows Matching Exact Photo -->
                  <div style="display:flex; flex-direction:column;">
                    
                    <div style="display:flex; justify-content:space-between; align-items:center; padding:12px 0; border-bottom:1px solid var(--border-color);">
                      <span style="font-size:0.92rem; color:var(--text-muted); font-weight:700;">النوع</span>
                      <span style="font-size:0.95rem; font-weight:800; color:var(--text-main);">المنهج الدراسي</span>
                    </div>

                    <div style="display:flex; justify-content:space-between; align-items:center; padding:12px 0; border-bottom:1px solid var(--border-color);">
                      <span style="font-size:0.92rem; color:var(--text-muted); font-weight:700;">تاريخ البدء</span>
                      <span style="font-size:0.95rem; font-weight:800; color:var(--text-main);">${startDateText}</span>
                    </div>

                    <div style="display:flex; justify-content:space-between; align-items:center; padding:12px 0; border-bottom:1px solid var(--border-color);">
                      <span style="font-size:0.92rem; color:var(--text-muted); font-weight:700;">تاريخ الانتهاء</span>
                      <span style="font-size:0.95rem; font-weight:800; color:var(--text-main);">${endDateText}</span>
                    </div>

                    <div style="display:flex; justify-content:space-between; align-items:center; padding:12px 0; border-bottom:1px solid var(--border-color);">
                      <span style="font-size:0.92rem; color:var(--text-muted); font-weight:700;">عدد الحصص</span>
                      <span style="font-size:0.95rem; font-weight:800; color:var(--text-main);">${totalSessions}</span>
                    </div>

                    <div style="display:flex; justify-content:space-between; align-items:center; padding:12px 0; border-bottom:1px solid var(--border-color);">
                      <span style="font-size:0.92rem; color:var(--text-muted); font-weight:700;">مدة الحصة</span>
                      <span style="font-size:0.95rem; font-weight:800; color:var(--text-main);">${sessionDuration} دقيقةً</span>
                    </div>

                    <div style="display:flex; justify-content:space-between; align-items:center; padding:12px 0; border-bottom:1px solid var(--border-color);">
                      <span style="font-size:0.92rem; color:var(--text-muted); font-weight:700;">إجمالي المقاعد</span>
                      <span style="font-size:0.95rem; font-weight:800; color:var(--text-main);">${maxSeats}</span>
                    </div>

                    <div style="display:flex; justify-content:space-between; align-items:center; padding:12px 0; border-bottom:1px solid var(--border-color);">
                      <span style="font-size:0.92rem; color:var(--text-muted); font-weight:700;">المقاعد المتبقية</span>
                      <span style="font-size:0.95rem; font-weight:800; color:${availableSeats <= 3 ? 'var(--error)' : 'var(--success)'};">${availableSeats}</span>
                    </div>

                    <div style="display:flex; justify-content:space-between; align-items:center; padding:12px 0; border-bottom:1px solid var(--border-color);">
                      <span style="font-size:0.92rem; color:var(--text-muted); font-weight:700;">سعر الحصة</span>
                      <span style="font-size:0.95rem; font-weight:800; color:var(--text-main);">${sessionPrice} ج.م.</span>
                    </div>

                    <div style="display:flex; justify-content:space-between; align-items:center; padding:12px 0; border-bottom:1px solid var(--border-color);">
                      <span style="font-size:0.92rem; color:var(--text-muted); font-weight:700;">نظام الدفع</span>
                      <span style="font-size:0.95rem; font-weight:800; color:var(--text-main);">شهريًّا</span>
                    </div>
                  ...
                  <!-- Register Button (Same platform primary theme) -->
                  <div style="margin-top:24px;">
                    ${isClosed ? `
                      <div style="width:100%; padding:14px 20px; border-radius:30px; font-size:1rem; font-weight:900; background:#9ca3af; color:#fff; text-align:center;">
                        🔒 المجموعة مغلقة وبدأت الدراسة
                      </div>
                    ` : isPendingInSelected ? `
                      <div style="width:100%; padding:14px 20px; border-radius:30px; font-size:1.05rem; font-weight:900; background:rgba(245, 158, 11, 0.12); border:2px solid #f59e0b; color:#d97706; text-align:center; display:flex; align-items:center; justify-content:center; gap:8px;">
                        <span>⏳ طلب التسجيل قيد المراجعة والاعتماد</span>
                      </div>
                    ` : isEnrolledInSelected ? `
                      <div style="width:100%; padding:14px 20px; border-radius:30px; font-size:1.05rem; font-weight:900; background:rgba(16,185,129,0.12); border:2px solid #10b981; color:#059669; text-align:center; display:flex; align-items:center; justify-content:center; gap:8px;">
                        <span>✓ أنت مسجل في هذه المجموعة</span>
                      </div>
                    ` : isFull ? `
                      <button disabled style="width:100%; padding:14px 20px; border-radius:30px; font-size:1.05rem; font-weight:900; background:#ef4444; color:#fff; border:none; cursor:not-allowed;">
                        🔒 اكتملت جميع المقاعد
                      </button>
                    ` : `
                      <button id="enroll-group-action-btn" data-group-id="${selectedGroup.id}" class="btn-primary"
                        style="width:100%; padding:14px 20px; border-radius:30px; font-size:1.15rem; font-weight:900; font-family:'Cairo',sans-serif; justify-content:center; box-shadow:0 6px 20px var(--primary-glow); transition:all 0.2s ease;">
                        سجل الآن 🚀
                      </button>
                    `}
                  </div>
          ...
          <div style="max-width: 1200px; margin: 0 auto; padding: 0 24px 60px 24px;">
            ...
            <div class="glass-card" style="padding: 40px; text-align:center; border-radius:20px;">
  ...
  bindEvents() {
    ...
    // Group Registration Button Event
    const enrollGroupBtn = this.container.querySelector("#enroll-group-action-btn");
    if (enrollGroupBtn) {
      enrollGroupBtn.addEventListener("click", async () => {
