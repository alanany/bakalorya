...
export default class CoursesView {
  ...
  renderCurriculumExplorer() {
    ...
    if (isStudent) {
      ...
      contentArea.innerHTML = `
        ...
        <!-- Suggested Groups Section for Current Grade (Same as Home) -->
          ...
          <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:14px; flex-wrap:wrap; gap:12px;">
            <div>
              <h3 style="font-size:1.15rem; font-weight:800; margin:0; color:var(--text-main); display:flex; align-items:center; gap:8px;">
                ...
                <span id="student-suggested-groups-title">مجموعات مقترحة</span>
              </h3>
            ...
            <div style="display:flex; align-items:center; gap:12px; flex-wrap:wrap;">
              <span class="badge" style="background:rgba(99,102,241,0.08); color:var(--primary); font-size:0.72rem; font-weight:800; padding:4px 10px; border-radius:10px; display:inline-flex; align-items:center; gap:5px;">
              ...
              <div style="position:relative; width:100%; max-width:260px;">
                <i data-lucide="search" style="position:absolute; right:14px; top:50%; transform:translateY(-50%); width:15px; height:15px; color:var(--text-muted); pointer-events:none;"></i>
                <input type="text" id="courses-page-search-input" placeholder="ابحث عن مجموعة أو معلم..." value="${this.searchQuery || ''}"
                  style="width:100%; padding:8px 36px 8px 12px; border-radius:30px; border:1.5px solid var(--border-color); background:var(--bg-card); color:var(--text-main); font-size:0.85rem; font-weight:600; outline:none; box-sizing:border-box;">
              </div>
            </div>
          </div>

          <div id="student-suggested-groups-carousel-mount">
            <!-- Populated dynamically by renderStudentSuggestedGroupsCarousel -->
          </div>
        </div>
  ...
  renderTeacherCoursesDashboard(contentArea) {
    ...
    renderFilteredCoursesList() {
      if (state.user && state.user.role === "student") {
        this.renderStudentSuggestedGroupsCarousel();
        return;
      }
  ...
  async initTeacherCurriculumSelector(preselectedGradeId = null, preselectedSubjectId = null) {
    ...
    renderStudentSuggestedGroupsCarousel() {
      const mount = this.container.querySelector("#student-suggested-groups-carousel-mount");
      if (!mount) return;
      ...
      const enrolledGroupIds = new Set(
        (this.enrollments || []).map(e => e.groupId).filter(Boolean)
      );

      const matchingCourses = (this.courses || []).filter(c => {
        const degree = (c.degree || "").toLowerCase();
        const matchesGrade = !this.explorerGradeId ||
          (c.grade?.id === this.explorerGradeId) ||
          (c.gradeId === this.explorerGradeId) ||
          (gradeName && degree.includes(gradeName.toLowerCase()));
        return matchesGrade;
      });
          ...
          course.groups.forEach(g => {
            if (!enrolledGroupIds.has(g.id)) {
              availableGradeGroups.push({
                ...
                gradeName: currentGrade?.name || course.grade?.name || "المرحلة الدراسية",
      ...
      let filteredGroups = availableGradeGroups;
      if (this.searchQuery) {
        const q = this.searchQuery.toLowerCase();
        filteredGroups = availableGradeGroups.filter(g => {
          const name = (g.name || "").toLowerCase();
          const cTitle = (g.course?.title || "").toLowerCase();
          const tName = (g.teacher?.name || "").toLowerCase();
          const sName = (g.subjectName || "").toLowerCase();
          return name.includes(q) || cTitle.includes(q) || tName.includes(q) || sName.includes(q);
        });
      }

      const titleEl = this.container.querySelector("#student-suggested-groups-title");
      if (titleEl) {
        titleEl.textContent = `مجموعات مقترحة (${filteredGroups.length})`;
      }

      if (filteredGroups.length === 0) {
        mount.innerHTML = `
          <div class="glass-card" style="padding:32px 20px; text-align:center; border-radius:18px; border:1px dashed rgba(99,102,241,0.3); background:rgba(99,102,241,0.02); display:flex; flex-direction:column; align-items:center; gap:10px;">
            ...
            <div>
              <h4 style="font-size:0.98rem; font-weight:800; margin:0 0 4px 0; color:var(--text-main);">
                ${this.searchQuery ? 'لا توجد مجموعات مطابقة لبحثك' : 'لا توجد مجموعات مقترحة جديدة متاحة حالياً لهذا الصف'}
              </h4>
              <p style="font-size:0.82rem; color:var(--text-muted); margin:0; max-width:460px; line-height:1.5;">
                ${this.searchQuery ? 'جرّب كتابة كلمات بحث أخرى أو مسح حقل البحث.' : 'المجموعات المقترحة تظهر فور توفر مقاعد بمجموعات جديدة تناسب مرحلتك الدراسية.'}
              </p>
            </div>
          </div>
        `;
      ...
      mount.innerHTML = `
        <div class="suggested-groups-carousel-wrapper" style="position:relative; width:100%;">
          ...
          <div id="suggested-groups-scroll-track" class="suggested-groups-scroll-container">
            ${filteredGroups.map(g => this.renderGradeGroupCard(g)).join('')}
          </div>
      ...
      // Scroll behavior & auto scroll setup
      const scrollTrack = mount.querySelector("#suggested-groups-scroll-track");
      if (scrollTrack) {
        let isPaused = false;
        const triggerAutoScroll = () => {
          if (isPaused || !scrollTrack) return;
          const maxScroll = scrollTrack.scrollWidth - scrollTrack.clientWidth;
          if (Math.abs(scrollTrack.scrollLeft) >= maxScroll - 15) {
        ...
        const carouselWrapper = mount.querySelector(".suggested-groups-carousel-wrapper") || scrollTrack;
        ...
        mount.querySelector("#btn-scroll-suggested-right")?.addEventListener("click", () => {
          isPaused = true;
          scrollTrack.scrollBy({ left: 335, behavior: "smooth" });
          setTimeout(() => { isPaused = false; }, 3500);
        });

        mount.querySelector("#btn-scroll-suggested-left")?.addEventListener("click", () => {
          isPaused = true;
          scrollTrack.scrollBy({ left: -335, behavior: "smooth" });
          setTimeout(() => { isPaused = false; }, 3500);
        });
