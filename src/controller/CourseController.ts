...
export class CourseController {
  ...
  static async create(req: AuthRequest, res: Response) {
    ...
    try {
      ...
      if (gradeId) {
        course.grade = { id: gradeId } as any;
      }
      if (subjectId) {
        course.subject = { id: subjectId } as any;
      }
  ...
  static async update(req: AuthRequest, res: Response) {
    ...
    try {
      ...
      if (gradeId !== undefined) {
        course.grade = gradeId ? ({ id: gradeId } as any) : null;
      }
      if (subjectId !== undefined) {
        course.subject = subjectId ? ({ id: subjectId } as any) : null;
      }
  ...
  // Add / register a unit
  static async addUnit(req: AuthRequest, res: Response) {
    const { id } = req.params;
  ...
  static async renameUnit(req: AuthRequest, res: Response) {
    const { id } = req.params;
  ...
  static async reorderUnits(req: AuthRequest, res: Response) {
    const { id } = req.params;
    ...
    try {
      ...
      for (const item of lessons) {
        if (item.id) {
          ...
          if (l) {
            if (typeof item.order === "number") l.order = item.order;
            if (item.chapter) l.chapter = item.chapter;
            await lessonRepo.save(l);
          }
        }
      }
