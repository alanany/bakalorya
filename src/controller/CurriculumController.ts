...
export class CurriculumController {
  ...
  static async seedEgyptianCurriculum(): Promise<void> {
    try {
      ...
      // Seed sample groups for demo matching screenshot if no groups exist
      const groupsCount = await groupRepo.count();
      if (groupsCount === 0) {
        console.log("🌱 Creating sample teachers, courses, and groups matching screenshot...");
        
        // Find Grade 2 Primary & Subject اللغة العربية
        const grade2 = await gradeRepo.findOne({ where: { code: "PRI_2" } });
        const subjectArabic = grade2 ? await subjectRepo.findOne({ where: { grade: { id: grade2.id }, name: "اللغة العربية" } }) : null;

        // Create Teacher 1: أمنية خالد
        let teacher1: any = await userRepo.findOne({ where: { email: "omnia.khaled@bakalorya.com" } });
        if (!teacher1) {
          teacher1 = userRepo.create({
            name: "أمنية خالد",
            email: "omnia.khaled@bakalorya.com",
            role: "teacher",
            avatar: "https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?w=200&auto=format&fit=crop&q=80",
            location: "القاهرة، مصر"
          });
          teacher1 = await userRepo.save(teacher1);
        }

        // Create Teacher 2: شيماء كمال
        let teacher2: any = await userRepo.findOne({ where: { email: "shaimaa.kamal@bakalorya.com" } });
        if (!teacher2) {
          teacher2 = userRepo.create({
            name: "شيماء كمال",
            email: "shaimaa.kamal@bakalorya.com",
            role: "teacher",
            avatar: "https://images.unsplash.com/photo-1544717305-2782549b5136?w=200&auto=format&fit=crop&q=80",
            location: "الجيزة، مصر"
          });
          teacher2 = await userRepo.save(teacher2);
        }

        if (subjectArabic && grade2) {
          // Course 1 by أمنية خالد
          let course1 = courseRepo.create({
            title: "كورس اللغة العربية المتميز - الصف الثاني الابتدائي",
            description: "شرح مبسط وتأسيس قوي في مهارات القراءة والنحو والإملاء لمنهج الصف الثاني الابتدائي مع أستاذة أمنية خالد.",
            category: "اللغة العربية",
            grade: grade2,
            subject: subjectArabic,
            teacher: teacher1,
            price: 320,
            isFree: false,
            currency: "ج.م.",
          ...
          course1 = await courseRepo.save(course1);

          // Group 1: الأحد والثلاثاء الساعة 6:00م (12 enrolled, 13 available out of 25)
          const group1 = groupRepo.create({
            name: "مجموعة الأحد والثلاثاء - مسائي",
            course: course1,
            teacher: teacher1,
            scheduleDays: "الأحد، الثلاثاء",
            scheduleTime: "6:00م",
            scheduleText: "الأحد والثلاثاء الساعة 6:00م",
            maxStudents: 25,
            status: "OPEN"
          });
          const savedGroup1 = await groupRepo.save(group1);

          // Create dummy enrollments to simulate 12 enrolled
          for (let i = 0; i < 12; i++) {
            const dummyStudent: any = await userRepo.findOne({ where: { email: "student@bakalorya.com" } });
            if (dummyStudent && i === 0) {
          ...
          // Course 2 by شيماء كمال
          let course2 = courseRepo.create({
            title: "شرح وتدريبات اللغة العربية المنهج الجديد - الصف الثاني الابتدائي",
            description: "متابعة أسبوعية واختبارات دورية مع أستاذة شيماء كمال لضمان تفوق الطالب وإتقان مهارات التعبير.",
            category: "اللغة العربية",
            grade: grade2,
            subject: subjectArabic,
            teacher: teacher2,
          ...
          course2 = await courseRepo.save(course2);

          // Group 2: الأحد والثلاثاء الساعة 6:00م (18 enrolled, 7 available out of 25)
          const group2 = groupRepo.create({
          ...
          const savedGroup2 = await groupRepo.save(group2);
        }

        // Also add sample group for 3rd Secondary Physics
        const gradeSec3 = await gradeRepo.findOne({ where: { code: "SEC_3" } });
        const subjectPhysics = gradeSec3 ? await subjectRepo.findOne({ where: { grade: { id: gradeSec3.id }, name: "الفيزياء" } }) : null;
        if (gradeSec3 && subjectPhysics) {
          let coursePhysics = courseRepo.create({
            title: "كورس الفيزياء للثانوية العامة 2026 - مستر محمد عناني",
            description: "شرح وافٍ وتدريب على جميع أفكار المسائل والفيزياء الحديثة وإعداد الطالب لامتحان الثانوية العامة.",
            category: "الفيزياء",
            grade: gradeSec3,
          ...
          coursePhysics = await courseRepo.save(coursePhysics);

          await groupRepo.save(groupRepo.create({
            name: "مجموعة السبت والأربعاء - 8:00م",
            course: coursePhysics,
            teacher: teacher1,
            scheduleDays: "السبت، الأربعاء",
            scheduleTime: "8:00م",
            scheduleText: "السبت والأربعاء الساعة 8:00م",
            maxStudents: 25,
            status: "OPEN"
          }));
        }
